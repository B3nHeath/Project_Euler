...
class Puzzle:
    ...
    def check_singles(self):
        # For each row and column, process hidden and naked singles
        for i in range(9):
            self.naked_singles(self.matrix[i], 'row', i)
            self.hidden_singles(self.matrix[i], 'row', i)
            self.naked_singles([self.matrix[x][i] for x in range(9)], 'col', i)
            self.hidden_singles([self.matrix[x][i] for x in range(9)], 'col', i) 
        # For each box, process hideen and naked singles
        for box in self.boxes.values():
            self.naked_singles([self.matrix[y][x] for y,x in box], "box", box)
    ...
    def check_pairs(self):
        for i in range(9):
            ...
            self.hidden_pairs(self.matrix[i], 'row', i)
            self.naked_pairs([self.matrix[x][i] for x in range(9)], 'col', i)
            self.hidden_pairs([self.matrix[x][i] for x in range(9)], 'col', i)
        ...
        for box in self.boxes.values():
            ...
            self.hidden_pairs([self.matrix[y][x] for y,x in box], "box", box)


    def naked_singles(self, cells, container_type, container_num):
    ...
    def hidden_pairs(self, cells, container_type, container_num):
        # Collect number of occurrences of each number
        counts = count_elements_lists(flatten_list(cells))
        double_values = []
        # For each value
        for value in counts.keys():
            # If present twice, add to list of double values
            if counts[value] == 2:
                double_values.append(value)

        # Create list of tuples of all possible combos for double values
        possible_combinations = [combo for combo in combinations(double_values, 2)]

        # For each possible combo
        for num1, num2 in possible_combinations:
            count = 0
            indexes = []
            # Go through cells in container
            for index, cell in enumerate(cells):
                if isinstance(cell, list):
                    # If dpuble values are contained in cell
                    if {num1,num2}.issubset(cell):
                        # Add 1 to frequency
                        count +=1
                        # Add index position
                        indexes.append(index)
            # If it occurs twice, we have identified a hidden pair
            if count == 2:
                # Update each of the relevant cells
                for index in indexes:
                    self.update_matrix([num1, num2], container_type, container_num, index)
...
def main():
    ...
    numbers = "001007090590080001030000080000005800050060020004100000080000030100020079020700400"
